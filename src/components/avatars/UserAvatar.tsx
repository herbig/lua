...
export function UserAvatar({ address, ...props }: Props) {
  const hash = useGetUserValue(address, 'avatar_img');
  ...
  // list is updated...
  return (
    <Avatar {...props} backgroundColor={bg} ignoreFallback={true} src={hash ? `https://cloudflare-ipfs.com/ipfs/${hash}` : undefined} />
  );
}